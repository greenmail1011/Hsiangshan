...
學生名單的備份檔（`香山國小分組-名單備份.json`）**不要**上傳到 GitHub。

## 目前進度（2026-09-24）

| 步驟 | 狀態 |
|---|---|
| 1. 建立 Firebase 專案 | 已完成：`xiangshan-groups` |
| 2. 建立 Firestore 資料庫 | 已完成：位置 asia-east1（台灣） |
| 3. 貼上安全規則 | 已完成：免登入版，已測試會擋下格式不對的資料 |
| 4. 填好 `firebase-config.js` | 已完成 |
| 5. 上傳到 GitHub | **待做** |
| 6. 開啟 GitHub Pages | **待做** |
| 7. 把現有名單搬進來 | 已完成：14 位學生、分組、小組長、設定都在 Firestore 裡了 |

所以現在只要做第 5、6 步，打開網址就能直接用。下面 1～4、7 步留著給之後重建或換專案時參考。
